	...
	class AIO_Icon_Manager {
		...
		function create_config()
		{
			...
			if (!is_wp_error($json) && !empty($json))
			{
				...
				$font_original_name = $newname = "";
				
				$xml = simplexml_load_string($response);
				$font_attr = $xml->defs->font->attributes();
				$font_original_name = (string) $font_attr['id'];
				
				if (is_dir(trailingslashit($this->paths['fontdir']).(string) $font_attr['id'])){
					$newname = (string) $font_attr['id'].uniqid();

					$response = str_replace((string) $font_attr['id'], $newname, $response);
					WP_Filesystem();
					global $wp_filesystem;
					
					$wp_filesystem->put_contents(trailingslashit($this->paths['tempurl']).$newname.".svg", $response, FS_CHMOD_FILE);
					$this->svg_file = trailingslashit($this->paths['tempurl']).$newname.".svg";
					$json = str_replace((string) $font_attr['id'], $newname, $json);
					$font_attr['id'] = $newname;
				}
				...
				{
					$this->write_config();
					$this->re_write_css($font_original_name, $newname);
					$this->rename_files();
					$this->rename_folder();
					$this->add_font();
				}
