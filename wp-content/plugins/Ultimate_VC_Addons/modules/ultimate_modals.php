...
if(!class_exists('Ultimate_Modals'))
{
	class Ultimate_Modals
	{
		...
		function modal_shortcode($atts, $content = null)
		{
			...
			$html .= '<div id="'.esc_attr($modal_trgs_id).'" class="ult-modal-input-wrapper '.esc_attr($is_vc_49_plus).' '.esc_attr($init_extra_class).' '.esc_attr($css_modal_box).' '.esc_attr($ult_hide_modal).' " '.$keypress_controls.' '.$overlay_controls.' data-modal-type="'.esc_attr( $modal_on ).'">';
			...
			if($modal_on == "button"){
				...
				if ($icon_type != "none"){
					if ($modal_on_align != "center"){
						$box_icon = str_replace('class="modal-icon"', 'class="modal-icon" style="float:left;padding-right: .7em; " ', $box_icon);
						$btn_text = $box_icon . $btn_text;
					} else {
						$btn_text = '<div class="icon_wrapper">'.$box_icon.'</div>' . $btn_text;
					}
				}
				...
				if($btn_img !==''){
					...
					$icon_image_font_size = "2em";
					if($header_font_size != '') $icon_image_font_size = str_replace('desktop:','', $header_font_size);
					$header_style = "";
					if($header_text_color !== '')
						$header_style .= 'color:'.$header_text_color.';';
					
					$box_icon = str_replace('class="modal-icon"', 'class="modal-icon" style="position: absolute; top: 50%; left: 50%; transform: translate3d(-50%,-50%,0); pointer-events: none; font-size: '.$icon_image_font_size.' '.$header_style.' " ', $box_icon);
					
					$html .= $box_icon . '<img src="'.esc_url(apply_filters('ultimate_images', $img)).'" alt="'.esc_attr($btn_alt).'" data-class-id="content-'.esc_attr($uniq).'" class="ult-modal-img '.esc_attr($modal_class).' ult-align-'.esc_attr($modal_on_align).' ult-modal-image-'.esc_attr($el_class).'" '.$modal_data_class.'/>';
				}
			}
			...
			elseif($modal_on == "custom-selector") {
				...
				if ($icon_type != "none"){
					if ($modal_on_align != "center"){
						$box_icon = str_replace('class="modal-icon"', 'class="modal-icon" style="display: inline-block; " ', $box_icon);
						$read_text = $box_icon .' '. $read_text;
			...
			if($modal_title !== ''){
				...
				$html .= "\n\t\t\t\t<h3 ".$madal_heading_data_list.' class="ult_modal-title ult-responsive">'.$modal_title.'</h3>';
				$html .= "\n\t\t\t".'</div>';
			}
