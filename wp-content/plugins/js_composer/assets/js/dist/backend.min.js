...
// BEGIN OF TREETHEMES
vc.atts.colorpicker={init:function(param,$field){var $content=$field;$(".vc_color-control",$content).each(function(){var $alpha,$alpha_output,$pickerContainer,$control=$(this),value=$control.val().replace(/\s+/g,""),alpha_val=100;value.match(/rgba\(\d+\,\d+\,\d+\,([^\)]+)\)/)&&(alpha_val=100*parseFloat(value.match(/rgba\(\d+\,\d+\,\d+\,([^\)]+)\)/)[1])),$control.wpColorPicker({clear:function(event,ui){$alpha.val(100),$alpha_output.val("100%")},change:_.debounce(function(){$(this).trigger("change")},500)}),$pickerContainer=$control.closest(".wp-picker-container"),

window.treethemes_color_picker_value=value;
var treethemes_is_this_color_selected=value=='__USE_THEME_MAIN_COLOR__' ? ' checked=\'checked\' ' : '';
var treethemes_color_picker_id='_' + Math.random().toString(36).substr(2, 9);
var treethemes_output='<div class=\'use_theme_main_color_container '+window.treethemes_color_picker_id+'\' style=\'margin: 10px 0px; \'><label>Use Theme Main Color ? <input name=\'use_theme_main_color\' type=\'checkbox\' '+treethemes_is_this_color_selected+' onchange=\' if (jQuery(this).is(":checked")) { jQuery(this).closest(".use_theme_main_color_container").siblings().find("input.wp-color-picker").val("__USE_THEME_MAIN_COLOR__"); jQuery(this).closest(".use_theme_main_color_container").siblings().fadeOut(); } else { jQuery(this).closest(".use_theme_main_color_container").siblings().find("input.wp-color-picker").val(""); jQuery(this).closest(".use_theme_main_color_container").siblings().fadeIn(); } \' /></label></div>';
$pickerContainer.prepend($(treethemes_output));
if (value=='__USE_THEME_MAIN_COLOR__') $field.find('.use_theme_main_color_container').siblings().fadeOut();

$('<div class="vc_alpha-container"><label>Alpha: <output class="rangevalue">'+alpha_val+'%</output></label><input type="range" min="1" max="100" value="'+alpha_val+'" name="alpha" class="vc_alpha-field"></div>').appendTo($pickerContainer.addClass("vc_color-picker").find(".iris-picker")),$alpha=$pickerContainer.find(".vc_alpha-field"),$alpha_output=$pickerContainer.find(".vc_alpha-container output"),$alpha.bind("change keyup",function(){var alpha_val=parseFloat($alpha.val()),iris=$control.data("a8c-iris"),color_picker=$control.data("wp-wpColorPicker");$alpha_output.val($alpha.val()+"%"),iris._color._alpha=alpha_val/100,$control.val(iris._color.toString()),color_picker.toggler.css({backgroundColor:$control.val()})}).val(alpha_val).trigger("change");

if(typeof window.treethemes_color_picker_value != "undefined" && window.treethemes_color_picker_value=='__USE_THEME_MAIN_COLOR__') {
	jQuery('.use_theme_main_color_container.'+window.treethemes_color_picker_id+' input[name="use_theme_main_color"]').trigger('click').trigger('click');
}	
