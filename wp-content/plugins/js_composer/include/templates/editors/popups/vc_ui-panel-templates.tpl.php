			...
			<div class="vc_ui-panel-content vc_properties-list"
				...
				<div class="vc_panel-tabs">
					...
					foreach ( $categories_data as $key => $category ) :
						...
						/* TREETHEMES additon - library sidebar*/
						if( function_exists('treethemes_vc_library_cat_list') && esc_attr( $category['category'] ) == 'default_templates') {
							echo '<div class="library_categories">
								<ul>';
									$treethemes_library_cats = treethemes_vc_library_cat_list();
									foreach($treethemes_library_cats as $cat_name => $cat_sort_text) {
										echo '<li data-sort="'.$cat_sort_text.'">'.$cat_name.' <span class="count">0</span></li>';
									}
								echo '</ul>
							</div>';
						}
						/* TREETHEMES additon end */
