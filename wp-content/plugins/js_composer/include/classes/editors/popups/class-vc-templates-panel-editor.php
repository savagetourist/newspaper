...
class Vc_Templates_Panel_Editor {
	...
	public function getAllTemplates() {
		...
		if ( ! empty( $default_templates ) ) {
			$arr_category = array(
				...
				'category_name' => __( 'Hazel Templates', 'js_composer' ),
				'category_description' => __( 'Append predefined Hazel templates to the current layout', 'js_composer' ),
				'category_weight' => 10,
				/*TREETHEMES addition end*/
			);
			$category_templates = array();
	...
	public function loadDefaultTemplates() {
		...
		// TREETHEMES - needed for the frontend.
		if ( ! is_array( $this->default_templates ) || ! $this->default_templates ) {
	...
	public function renderTemplateListItem( $template ) {
		...
		/*TREETHEMES addition - adding in preview img*/
		$preview_img = esc_attr( isset( $template['image'] ) &&  $template['image'] != '' ? $template['image'] : get_template_directory_uri() .'/images/template-without-image.jpg');
